function AddRowsToProductTable(List, orderIdFromForm) {
    ...
    ProductTableData.each(function (i, el) {
        ...
        rowsData += "<td><a id = save_" + k + " onclick='PostAddItemToOrder(" + k + "," + orderIdFromForm+")'> <i class=\"s-24 icon-save red-blue\"></i></a ></td>";
        rowsData += "</tr>";
...
function PostAddItemToOrder(k, orderIdFromForm) {

    var jsonData = {
        orderId: orderIdFromForm,
        ProductCatalogId: $("#ProductCatalogName_" + k).val(),
        Quantity: $("#Quantity_" + k).val(),
        TaxAmount_: $("#TaxAmount__" + k).val(),
        UnitPriceExclTax: $("#UnitPriceExclTax_" + k).val(),
    };

    $.post("/Order/AddProductToOrderDetails",
...
function PostDeleteItemFromOrder(k, orderIdFromForm) {

    var jsonData = {
        Id: orderIdFromForm,
        orderItemId: $("#OrderItemId_" + k).val()
     
    };

    $.post("/Order/DeleteOrderItem",
...
function AddRowsToAssociateTable(k, List) {
    ...
    mailingListTableData.each(function (i, el) {
        ...
        rowsData += "<td><input name='Items[" + k + "].ProductId' id=ProductName_" + k + " class=\"form-control \" value=\"\"/></td>";
        rowsData += "<td><input name='Items[" + k + "].Quantity' id=SKU_" + k + " class=\"form-control \" value=\"\"/></td>";
        rowsData += "<td><input name='Items[" + k + "].Price' id=Price_" + k + " class=\"form-control \" value=\"\"/></td>";
    ...
    //    });
    //    var arrayData = List.data[arrayindex];
    //    document.getElementById("SKU_" + k + "").value = "1";
